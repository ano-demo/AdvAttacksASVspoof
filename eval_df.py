...
def main(config, resume, protocol_file, asv_score_file):
    ...
    data_loader = getattr(module_data, config['dev_data_loader']['type'])(
        "/data/longnv/trn_dev_eval_scps/ASVspoof2021_DF_eval.scp",
    ...
    with torch.no_grad():
        for i, (utt_list, data, target) in enumerate(tqdm(data_loader)):
            ...
            # logger.info({"utt_list":utt_list})
            
            for index, utt_id in enumerate(utt_list):
                utt2scores[utt_id] = score[index].item()
                # logger.info({"score[index].item()":score[index].item()})
                logger.info({"utt_id":utt_id, "index":index, "utt2score":utt2scores[utt_id]})
    ...
    logger.info({"EER": eer, "EER_point": eer_point})
